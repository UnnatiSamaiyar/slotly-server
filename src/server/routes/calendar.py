...
def refresh_access_token(refresh_token: str) -> Dict[str, Any]:
    ...
    if res.status_code != 200:
        raise HTTPException(status_code=502, detail=f"Google token refresh failed: {res.text}")
    return res.json()

@router.get("/calendar/events")
def get_calendar_events(user_sub: str, db: Session = Depends(get_db), max_results: int = 50):
    """
    ...
    events_res = requests.get(GOOGLE_CALENDAR_EVENTS, headers=headers, params=params, timeout=10)
    if events_res.status_code != 200:
        raise HTTPException(status_code=502, detail=f"Google calendar fetch failed: {events_res.text}")

    events_payload = events_res.json()
    items = events_payload.get("items", [])

    # simplify the events to fields your UI will use
    def simplify(evt: Dict[str, Any]) -> Dict[str, Any]:
        start = evt.get("start", {})
        end = evt.get("end", {})
        start_dt = start.get("dateTime") or start.get("date")  # dateTime for timed events, date for all-day
...
def list_calendars(user_sub: str, db: Session = Depends(get_db)):
    ...
    if res.status_code != 200:
        raise HTTPException(status_code=502, detail=f"Google calendar list failed: {res.text}")
