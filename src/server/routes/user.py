...
def get_me(user_sub: str, db: Session = Depends(get_db)):
    """
    Returns user profile + primary booking profile (if exists)
    """
    ...
    return {
        ...
        # booking profile info (only fields that exist)
        "profile_title": primary_profile.title if primary_profile else None,
        "host_name": user.name,
        "timezone": None,  # BookingProfile has no timezone column in your model
